...
// GPU (NVENC) transcoding — opt-in via STREAM_USE_GPU=true (set by docker-compose.gpu.yml),
// and only actually used if this ffmpeg build reports an nvenc encoder. Detected once at
// startup rather than per-job so a broken/missing GPU doesn't add a probe to every upload;
// each quality rendition still falls back to CPU (libx264) on its own if the GPU encode
// attempt fails at runtime (e.g. driver/session issue), so a bad GPU setup degrades
// transcoding speed rather than breaking uploads outright.
...
app.post('/upload', requireToken, upload.single('video'), async (req, res) => {
  ...
  // Queued, not started immediately — see the queue section below. Multiple uploads
  // (several videos' main sources + mirrors) used to all call transcodeToHLS() at once,
  // so their ffmpeg processes fought over the same CPU cores and everything got slower;
  // now only one ffmpeg encode ever runs at a time, in submission order.
