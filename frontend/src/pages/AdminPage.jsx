      ...
      {tab === 'videos' && (
        <div>
          ...
          {selectedIds.length > 0 && (
            <div className="mb-4 p-4 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl border border-indigo-200 dark:border-indigo-500/20 flex flex-wrap items-center gap-3 animate-slide-up">
              <span className="text-sm font-bold text-indigo-700 dark:text-indigo-300">Zaznaczono: {selectedIds.length}</span>
                ...
                <tbody>
                  {loading ? (
                    ...
                    filteredVideos.map(video => (
                      <tr key={video.id} className={`border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors ${selectedIds.includes(video.id) ? 'bg-indigo-50/50 dark:bg-indigo-500/5' : ''}`}>
                        <td className="w-10 px-3 py-3">
                          <button onClick={() => toggleSelect(video.id)} className="text-zinc-400 hover:text-indigo-500 transition-colors">
                            {selectedIds.includes(video.id) ? <CheckSquare className="w-4 h-4 text-indigo-500" /> : <Square className="w-4 h-4" />}
                          </button>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-xs font-bold ${video.category_id ? 'text-indigo-600 dark:text-indigo-400' : 'text-zinc-400'}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => { setEditingVideo(video); setIsModalOpen(true); }} className="p-1.5 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-lg transition-colors text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400">
      ...
      {tab === 'logs' && (
        <div>
          ...
          <div className="flex gap-2 mb-6">
            <button onClick={() => setLogSubTab('watch')} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${logSubTab === 'watch' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-white'}`}>
              <Eye className="w-4 h-4 inline mr-2" />Logi wyświetleń ({watchLogsMeta.total})
            </button>
            <button onClick={() => setLogSubTab('login')} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${logSubTab === 'login' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-white'}`}>
                  ...
                  <div className="flex gap-1">
                    {Array.from({ length: Math.min(watchLogsMeta.totalPages, 10) }, (_, i) => i + 1).map(p => (
                      <button key={p} onClick={() => { api.getWatchLogs(p).then(r => { setWatchLogs(r.logs); setWatchLogsMeta({ total: r.total, page: r.page, totalPages: r.totalPages }); }); }}
                        className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${p === watchLogsMeta.page ? 'bg-indigo-600 text-white' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>{p}</button>
                    ))}
                  ...
                  <div className="flex gap-1">
                    {Array.from({ length: Math.min(loginLogsMeta.totalPages, 10) }, (_, i) => i + 1).map(p => (
                      <button key={p} onClick={() => { api.getLoginLogs(p).then(r => { setLoginLogs(r.logs); setLoginLogsMeta({ total: r.total, page: r.page, totalPages: r.totalPages }); }); }}
                        className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${p === loginLogsMeta.page ? 'bg-indigo-600 text-white' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>{p}</button>
                    ))}
