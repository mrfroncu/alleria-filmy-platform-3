      ...
      {captureDetected && drmEnhanced && (
        <div className="absolute inset-0 bg-black z-50 flex items-center justify-center">
          <div className="text-center p-8">
            ...
            <button onClick={() => setCaptureDetected(false)} className="mt-6 px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 transition-colors">
              Sprawdź ponownie
            </button>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-t from-black/80 via-black/40 to-transparent pt-16 pb-4 px-5">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => { videoRef.current.muted = !muted; setMuted(!muted); }} className="p-1.5 text-white hover:text-indigo-400 transition-colors">
            ...
            <div className="flex items-center gap-2">
              ...
              {qualities.length > 1 && (
                <div className="relative">
                  <button onClick={() => setShowQuality(!showQuality)} className="p-1.5 text-white hover:text-indigo-400 transition-colors flex items-center gap-1">
                  ...
                  {showQuality && (
                    <div className="absolute bottom-full right-0 mb-2 bg-black/90 backdrop-blur rounded-xl border border-white/10 py-1 min-w-[120px]">
                      <button onClick={() => changeQuality(-1)} className={`w-full text-left px-4 py-2 text-sm ${currentQuality === -1 ? 'text-indigo-400 font-bold' : 'text-white'} hover:bg-white/10`}>
                        Auto
                      </button>
                      {qualities.map(q => (
                        <button key={q.index} onClick={() => changeQuality(q.index)} className={`w-full text-left px-4 py-2 text-sm ${currentQuality === q.index ? 'text-indigo-400 font-bold' : 'text-white'} hover:bg-white/10`}>
                          {q.label}
                        </button>
                      ))}
                    </div>
              ...
              <button onClick={toggleFullscreen} className="p-1.5 text-white hover:text-indigo-400 transition-colors">
