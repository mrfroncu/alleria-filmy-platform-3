// Baseline: the whole schema as it stood before numbered migrations existed (everything that
// used to run inline in database.js on every boot). Every statement is idempotent — CREATE ... IF
// NOT EXISTS, or ALTER/UPDATE wrapped in try/catch — so it is safe on both a brand-new database
// and on an existing production one, where it is a no-op that simply gets recorded as applied.
//
// Do NOT add to this file. New schema changes go in a new numbered file (see ./index.js).
module.exports = {
  version: 1,
  name: 'baseline',
  description: 'Schemat bazowy (stan sprzed wprowadzenia numerowanych migracji)',
  // Runs outside a transaction on purpose — exactly how this code always ran; each ALTER is its
  // own try/catch and some older databases rely on individual statements failing independently.
  transaction: false,
  up(db) {
    db.exec(`
